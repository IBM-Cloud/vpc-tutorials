...
resource aws_vpc "vpc" {
  tags = { name = "aws_vpc__vpc" }
  cidr_block = "10.0.0.0/16"
}

resource aws_internet_gateway "igw" {
  tags = { name = "aws_internet_gateway__igw" }
  vpc_id = "${aws_vpc.vpc.id}"
}

resource aws_route_table "public" {
  tags = { name = "aws_route_table__public" }
  vpc_id = "${aws_vpc.vpc.id}"
  route {
...
resource aws_subnet "public_0" {
  tags = { name = "aws_subnet__public_0" }
  vpc_id = "${aws_vpc.vpc.id}"
...
resource aws_subnet "public_1" {
  tags = { name = "aws_subnet__public_1" }
  vpc_id = "${aws_vpc.vpc.id}"
  availability_zone = "${var.aws_zones[0]}"
...
resource aws_security_group "sg1" {
  tags = { name = "aws_security_group__sg1" }
