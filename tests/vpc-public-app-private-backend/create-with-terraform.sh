...
ssh_insecure_config="$(cd $this_dir/../../scripts; pwd -P)"/ssh.insecure.config
function testit() {
    ...
    test_curl $BACK_NIC_IP "ssh -F "$ssh_insecure_config" -o ProxyJump=root@$BASTION_IP_ADDRESS root@$FRONT_NIC_IP" 'I am the backend server'
}
