...
locals {
  ...
  frontend_security_groups = "${split(",", var.maintenance ? format("%s,%s", ibm_is_security_group.frontend.id, module.bastion.security_group_id) : ibm_is_security_group.frontend.id)}"
}
...
resource "ibm_is_instance" "frontend" {
  ...
  primary_network_interface = {
    ...
    security_groups = ["${local.frontend_security_groups}"]
  }
}
...
locals {
  ...
  backend_security_groups = "${split(",", var.maintenance ? format("%s,%s", ibm_is_security_group.backend.id, module.bastion.security_group_id) : ibm_is_security_group.frontend.id)}"
}
