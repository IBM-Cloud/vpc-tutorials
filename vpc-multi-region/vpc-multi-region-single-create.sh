...
# App and VPN servers
echo "Creating VSIs"
VSI_ZONE1=$(ibmcloud is instance-create ${BASENAME}-$REGION-zone1-vsi $VPCID $REGION-1 $(instance_profile) $SUB_ZONE1_ID --image-id $ImageId --key-ids $SSHKey --security-group-ids $SG_ZONE1_ID,$SGMAINT  --json)
VSI_ZONE2=$(ibmcloud is instance-create ${BASENAME}-$REGION-zone2-vsi $VPCID $REGION-2 $(instance_profile) $SUB_ZONE2_ID --image-id $ImageId --key-ids $SSHKey --security-group-ids $SG_ZONE2_ID,$SGMAINT --json)
