...
GLB_ID=$(echo "$GLBS" | jq -r '.[] | select(.name=="lb1.'$DOMAIN_NAME'") .id')
ibmcloud cis glb-delete $DOMAIN_ID $GLB_ID -i $INSTANCE_NAME

POOLID=$(echo "$GLBS" | jq -r '.[] | select(.name=="lb1.'$DOMAIN_NAME'").default_pools[0]')
MONITORID=$(ibmcloud cis glb-pool $POOLID -i $INSTANCE_NAME --output json | jq -r '.monitor')

echo "Deleting pools..."
echo "$GLBS" | jq -c -r '.[] | select(.name=="lb1.'$DOMAIN_NAME'").default_pools[]' | while read poolid;
do
