...
resource "ibm_is_security_group_rule" "sg_database_admin_inbound_tcp_26257" {
  ...
  group     = "${ibm_is_security_group.sg_database.id}"
  direction = "inbound"
  remote    = "${ibm_is_subnet.sub_admin.ipv4_cidr_block}"
...
resource "ibm_is_lb_pool_member" "database_pool_members" {
  ...
  port           = 26257
  target_address = "${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}"
}
...
data "template_file" "cockroachdb_basic_systemd" {
  ...
  vars = {
    ...
    join_list             = "${join(",", ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address)}"
...
resource "null_resource" "vsi_database" {
  ...
  connection {
    type         = "ssh"
    host         = "${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}"
    user         = "root"
  ...
  provisioner "local-exec" {
    command = "scp -F ./scripts/ssh.config -i ${var.ssh_private_key} -o 'ProxyJump root@${ibm_is_floating_ip.vpc_vsi_admin_fip.0.address}' config/${var.resources_prefix}-certs/${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}.node.key root@${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}:/data/certs/node.key"
    interpreter = ["bash", "-c"]  
  }

  provisioner "local-exec" {
    command = "scp -F ./scripts/ssh.config -i ${var.ssh_private_key} -o 'ProxyJump root@${ibm_is_floating_ip.vpc_vsi_admin_fip.0.address}' config/${var.resources_prefix}-certs/${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}.node.crt root@${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}:/data/certs/node.crt"
    interpreter = ["bash", "-c"]  
  }

  provisioner "local-exec" {
    command = "scp -F ./scripts/ssh.config -i ${var.ssh_private_key} -o 'ProxyJump root@${ibm_is_floating_ip.vpc_vsi_admin_fip.0.address}' config/${var.resources_prefix}-certs/ca.crt root@${element(ibm_is_instance.vsi_database.*.primary_network_interface.0.primary_ipv4_address, count.index)}:/data/certs/ca.crt"
    interpreter = ["bash", "-c"]  
  }
